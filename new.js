...
app.post("/search", async (req, res) => {
  ...
  const [eloProducts, darazProducts] = await Promise.all([
    elo(query),
    daraz(query),
  ]);
  const jsonData = JSON.stringify({ daraz: darazProducts, elo: eloProducts });
  fs.writeFileSync("search_data.json", jsonData);
  res.json({ daraz: darazProducts, elo: eloProducts });
});
